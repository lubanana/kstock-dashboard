...
"""
STRICT 모드 백테스트
조건: Morning Star 또는 Bullish Engulfing만, 50점+
"""
...
ALL_STOCKS = [
    ...
    '032640', '033780',
    # 중형주
    '012330', '018260', '009540', '010130', '010950', '010955',
    '161890', '271560', '267250',
    # 반도체/테크
...
def scan_strict_mode(symbol: str, date: str) -> dict:
    """
    STRICT 모드 스캔
    조건:
    1. Morning Star 또는 Bullish Engulfing 패털만
    2. 최소 50점
    3. 피볼나치 30~70%
    4. 거래량 1.2x+
    """
    try:
        ...
        # 3. STRICT 패턴 확인 (Morning Star 또는 Bullish Engulfing만)
        all_patterns = detector.detect_all_patterns(min_reliability=4)
        
        valid_pattern = None
        for p in all_patterns:
            if p['pattern'] in ['morning_star', 'bullish_engulfing']:
                valid_pattern = p
                break
...
def simulate_strict(signal: dict) -> dict:
    """STRICT 모드 시뮬레이션"""
    try:
        entry = signal['price']
        ...
        for idx in range(min(max_days, len(df))):
            ...
            if row['Low'] <= stop:
                return {
                    'return': round((stop - entry) / entry * 100, 2),
                    'exit': 'stop',
                    'days': idx + 1
                }
            if row['High'] >= target:
...
def run_strict_backtest():
    """STRICT 모드 백테스트 실행"""
    print("=" * 80)
    print("🔥 STRICT 모드 백테스트")
    print("=" * 80)
    print("조건: Morning Star 또는 Bullish Engulfing만, 50점+")
    ...
    all_trades = []
    pattern_stats = {'morning_star': {'count': 0, 'wins': 0, 'returns': []},
                     'bullish_engulfing': {'count': 0, 'wins': 0, 'returns': []}}
    ...
    for symbol in ALL_STOCKS:
        ...
        for date in dates:
            signal = scan_strict_mode(symbol, date)
            if signal:
                total_signals += 1
                symbol_count += 1
                trade = simulate_strict(signal)
                if trade:
                    trade.update({
                        'symbol': symbol,
                        'date': date,
...
def analyze_strict_results(trades, pattern_stats, total_signals):
    """STRICT 모드 결과 분석"""
    print("\n" + "=" * 80)
    print("📊 STRICT 모드 결과 분석")
    print("=" * 80)
...
if __name__ == '__main__':
    trades, pattern_stats, total_signals = run_strict_backtest()
    results = analyze_strict_results(trades, pattern_stats, total_signals)
    
    if results:
        with open('docs/strict_mode_backtest.json', 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2, default=str)
        print("\n✅ 저장: docs/strict_mode_backtest.json")
