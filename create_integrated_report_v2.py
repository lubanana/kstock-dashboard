#!/usr/bin/env python3
"""
통합 프리미엄 리포트 생성기 (심층분석 포함)
KAGGRESSIVE + 단기 급등 통합 + 고득점 종목 심층분석
"""
...
def create_integrated_report_with_deep_analysis():
    """통합 리포트 생성 (심층분석 포함)"""
    
    print('='*70)
    print('🔥 통합 프리미엄 리포트 + 심층분석')
    print('='*70)
    ...
    html = generate_html_report_with_analysis(tier1, tier2, deep_analysis, date_str)
    ...
    return tier1, tier2, deep_analysis


def generate_html_report_with_analysis(tier1, tier2, deep_analysis, date_str):
    """HTML 리포트 생성 (심층분석 포함)"""
        ...
        if deep:
            ...
            <div style="margin-top: 15px; padding: 12px; background: rgba(0,255,136,0.05); border-radius: 8px; border: 1px solid rgba(0,255,136,0.2);">
                ...
                <div style="color: #adb5bd; font-size: 12px; margin-bottom: 8px;">
                    키워드: {', '.join(sentiment['keywords'])}
                </div>
                <div style="color: #fff; font-size: 12px; line-height: 1.5;">
                    💬 {sentiment['community_mood']}
                </div>
            </div>
            """
    ...
    <div class="container">
        ...
        <div class="section tier1">
            <div class="section-title">
                ...
                <span style="font-size: 14px;">총 {len(tier1)}개 + 심층분석</span>
            </div>
        ...
        <div class="footer">
            ...
            <p style="margin-top: 10px; color: #495057;">심층분석: 네이버 증권 연동 | 여론 분석 포함</p>
        </div>
    </div>
</body>
</html>"""
...
if __name__ == '__main__':
    tier1, tier2, deep = create_integrated_report_with_deep_analysis()
    ...
    for r in tier1:
        d = next((x for x in deep if x['symbol'] == r['symbol']), None)
        if d:
            print(f"   • {r['name']} ({r['symbol']}): {r['score']}점")
            print(f"     여론: {d['sentiment']['overall']} ({d['sentiment']['score']}/100)")
            print(f"     링크: 네이버 종합/뉴스/종토방")
