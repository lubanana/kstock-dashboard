...
class CustomStockAnalyzer:
    ...
    def analyze_stock(self, symbol: str, name: str, market: str = 'KOSPI') -> Dict:
        ...
        try:
            ...
            print(f"      ⚠️  Error: {e}")
            result['agents']['technical'] = {'error': str(e)}
        ...
        try:
            ...
            print(f"      ⚠️  Error: {e}")
            result['agents']['quant'] = {'error': str(e)}
        ...
        try:
            ...
            result['agents']['qualitative'] = {
                'score': qual_result.get('total_score', 0),
                'recommendation': qual_result.get('recommendation', 'HOLD'),
                'moat': qual_result.get('moat_rating', 'None'),
                'signals': qual_result.get('key_signals', []),
                'risks': qual_result.get('key_risks', [])
            }
            print(f"      ✅ Score: {qual_result.get('total_score', 0)}/100")
        except Exception as e:
            print(f"      ⚠️  Error: {e}")
            result['agents']['qualitative'] = {'error': str(e)}
        
        # 4. News Sentiment
        print("\n   📰 News Sentiment Analysis...")
        try:
            news_agent = EnhancedNewsAgent(symbol, name)
            news_result = news_agent.analyze()
            result['agents']['news'] = {
                'score': news_result.get('total_score', 0),
                'recommendation': news_result.get('recommendation', 'HOLD'),
                'sentiment_trend': news_result.get('sentiment_trend', 'Stable'),
                'signals': news_result.get('key_signals', []),
                'risks': news_result.get('key_risks', [])
            }
            print(f"      ✅ Score: {news_result.get('total_score', 0)}/100")
        except Exception as e:
            print(f"      ⚠️  Error: {e}")
            result['agents']['news'] = {'error': str(e)}
...
def main():
    """메인 실행"""
    print("=" * 60)
    print("📊 Custom Stock Analysis Report")
    print("=" * 60)
