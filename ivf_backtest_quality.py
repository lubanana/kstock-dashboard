...
from ivf_scanner_quality import calculate_quality_signal


class IVFQualityBacktest:
    """고품질 IVF 백테스트"""
    ...
    def run(self, symbols, dates):
        """백테스트 실행"""
        print("=" * 70)
        print("🔥 IVF 고품질 전략 백테스트")
        print("   조건: 확실한 케이스만 (70점+ 권장)")
        print("=" * 70)
        ...
        for symbol in symbols:
            for date in dates:
                signal = calculate_quality_signal(symbol, date)
                if signal:
        ...
        print(f"   → 70점+ 고품질: {len(high_quality)}개")
        
        # 60~69점
        medium_quality = [s for s in all_signals if 60 <= s[2]['score'] < 70]
        print(f"   → 60~69점 중간: {len(medium_quality)}개")
        
        # 테스트: 두 그룹 모두
        test_signals = high_quality + medium_quality
        
        if not test_signals:
            print("\n⚠️ 테스트할 신호가 없습니다.")
            return None
        
        print(f"\n📈 {len(test_signals)}개 신호로 시뮬레이션 시작")
        print("-" * 70)
    ...
    def analyze(self):
        ...
        # 점수별
        print(f"\n🎯 점수별 성과")
        for threshold, name in [(80, '80+'), (70, '70~79'), (60, '60~69')]:
            subset = [t for t in self.trades if t['signal_score'] >= threshold 
                     and t['signal_score'] < (threshold + 10 if threshold < 80 else 100)]
            if subset:
        ...
        print(f"\n⚠️ TOP 3 손실")
        for t in sorted(self.trades, key=lambda x: x['return_pct'])[:3]:
...
if __name__ == '__main__':
    ...
    if results:
        with open('docs/ivf_quality_backtest_results.json', 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2, default=str)
        print("\n✅ 저장: docs/ivf_quality_backtest_results.json")
