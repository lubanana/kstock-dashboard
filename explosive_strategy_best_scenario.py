#!/usr/bin/env python3
"""
한국 주식 폭발적 상승 전략 - 최적 시나리오 H 구현
================================================
시나리오 H: ADX 필터 + 손익조정 + 85점+ 엄격 진입 (복합)
...
핵심 개선사항:
1. 진입 기준: 60점 → 85점+ (엄격 상향)
2. 기술적 필터: ADX > 20 + 정배열 필수
3. 손익 관리: 익절 +25%, 손절 -7%
4. 포지션 관리: 최대 8개, 점수별 가중치
"""
...
@dataclass
class StockScore:
    """개선된 스코어링 시스템 - 최적 가중치 적용"""
    code: str
    name: str
    date: str
    
    # 최적화된 가중치
    volume_score: int = 0      # 35점 (상향)
    technical_score: int = 0   # 20점 (유지)
    market_cap_score: int = 0  # 10점 (하향)
    foreign_score: int = 0     # 10점 (유지)
    ...
    def is_adx_valid(self) -> bool:
        """ADX > 20 확인 (강한 추세)"""
        return self.adx > 20 or self.adx == 0  # ADX 계산 불가시 통과
...
class ExplosiveStrategyBestScenario:
    """
    최적 시나리오 H 백테스트 엔진
    """
    ...
    # === 최적 파라미터 설정 (시나리오 H) ===
    MAX_HOLD_DAYS = 126           # 최대 6개월
    STOP_LOSS_PCT = -7.0          # -7% 손절 (강화)
    TAKE_PROFIT_PCT = 25.0        # +25% 익절 (조정)
    REBALANCE_DAY = 1             # 매월 1일 리밸런싱
    MAX_POSITIONS = 8             # 최대 8개 종목 (조정)
    
    # === 진입 기준 (엄격) ===
    ENTRY_SCORE_MIN = 85          # 85점+만 진입
    RSI_MIN = 40                  # RSI 최소
    RSI_MAX = 70                  # RSI 최대
    ADX_MIN = 20                  # ADX 최소
    MA_ALIGNMENT_REQUIRED = True  # 정배열 필수
    
    # === 포지션 사이징 (세분화) ===
    POSITION_WEIGHTS = {
        90: 25.0,   # 90점+: 25%
        85: 20.0,   # 85-89점: 20%
        80: 15.0,   # 80-84점: 15% (백테스트용, 실제는 85+만)
        75: 10.0    # 75-79점: 10% (백테스트용)
    }
    
    # === 섹터 분산 ===
    MAX_SECTOR_EXPOSURE = 30.0    # 동일 섹터 최대 30%
    ...
    def _get_price_data(self, code: str, start_date: str, end_date: str) -> pd.DataFrame:
        """가격 데이터 조회 (캐싱)"""
        cache_key = f"{code}_{start_date}_{end_date}_h"
    ...
    def _calculate_stock_score(self, code: str, date: str) -> Optional[StockScore]:
        ...
        if latest['close'] > score.ma20:
            ...
            # 정배열
            if score.ma5 > score.ma20 > score.ma60 > 0:
                tech_score += 7  # 완전 정배열
            elif score.ma5 > score.ma20:
                tech_score += 4  # 부분 정배열
        ...
        if market_cap <= 1000:
            ...
            score.market_cap_score = 7
        elif market_cap <= 5000:
            score.market_cap_score = 4
        else:
        ...
        self.score_history.append({
            ...
            'rsi': score.rsi,
            'adx': score.adx,
            'ma_aligned': score.is_ma_aligned(),
            'passed_filter': self._pass_entry_filter(score)
        })
        
        return score
    
    def _pass_entry_filter(self, score: StockScore) -> bool:
        """진입 필터 통과 여부"""
        # 1. 최소 점수 체크
        ...
        # 4. ADX 체크 (선택적)
        if score.adx > 0 and score.adx < self.ADX_MIN:
            return False
    ...
    def _get_position_weight(self, score: int) -> float:
        ...
        if score >= 90:
            ...
            return self.POSITION_WEIGHTS[75]
        return 0.0
    ...
    def _get_sector_exposure(self, sector: str) -> float:
        ...
        for code, trade in self.positions.items():
            info = self.stock_info.get(code, {'sector': '기타'})
            if info['sector'] == sector:
                exposure += trade.position_pct
        return exposure
    
    def _can_enter_sector(self, sector: str, weight: float) -> bool:
        """섹터 진입 가능 여부"""
        current = self._get_sector_exposure(sector)
        return (current + weight) <= self.MAX_SECTOR_EXPOSURE
    ...
    def _scan_stocks(self, date: str) -> List[StockScore]:
        ...
        for code in codes:
            ...
            if score and self._pass_entry_filter(score):
    ...
    def _execute_rebalance(self, date: str, top_scores: List[StockScore]):
        """리밸런싱 실행"""
        selected = [s for s in top_scores if self._pass_entry_filter(s)]
        selected = selected[:self.MAX_POSITIONS]
        
        if not selected:
            print(f"   ⚠️ 진입 대상 없음 (필터 통과 종목: 0개)")
            return
        ...
        for score in selected:
            ...
            if not self._can_enter_sector(score.sector, weight):
                print(f"   ⚠️ {score.name}: 섹터 한도 초과 ({score.sector})")
                continue
            ...
            if shares > 0:
                ...
                self.sector_exposure[score.sector] += weight
                print(f"   ✅ 진입: {score.name} ({score.code}) @ {score.close:,.0f}원 "
                      f"(점수: {score.total_score}, 비중: {weight}%, 섹터: {score.sector})")
    ...
    def _close_position(self, code: str, date: str, reason: str, exit_price: Optional[float] = None):
        ...
        self.sector_exposure[trade.sector] -= trade.position_pct
    ...
    def run_backtest(self, start_date: str = '2023-01-01', end_date: str = '2025-12-31') -> Dict:
        """백테스트 실행"""
        print("=" * 80)
        print("🔥 최적 시나리오 H 백테스트 실행")
        print("=" * 80)
        print(f"📅 기간: {start_date} ~ {end_date}")
        print(f"💰 초기 자본: {self.initial_capital:,.0f}원")
        print()
        print("[시나리오 H 핵심 설정]")
        print(f"  • 진입 기준: {self.ENTRY_SCORE_MIN}점+ (엄격)")
        print(f"  • RSI 필터: {self.RSI_MIN}~{self.RSI_MAX}")
        print(f"  • ADX 필터: {self.ADX_MIN}+")
        print(f"  • 정배열: 필수")
        print(f"  • 손절: {self.STOP_LOSS_PCT}% / 익절: +{self.TAKE_PROFIT_PCT}%")
        print(f"  • 최대 포지션: {self.MAX_POSITIONS}개")
        print(f"  • 섹터 한도: {self.MAX_SECTOR_EXPOSURE}%")
        print()
        print("=" * 80)
        ...
        for i, date in enumerate(trading_days):
            ...
            if year_month != current_month:
                ...
                month_trades = []
                print(f"\n📅 {year_month} 시작 (포트폴리오: {month_start_value:,.0f}원)")
            ...
            if date_dt.day == self.REBALANCE_DAY or i == 0:
                ...
                if top_scores:
                    print(f"   🎯 85점+ 종목: {len(top_scores)}개")
                    for s in top_scores[:5]:
                        print(f"      - {s.name}: {s.total_score}점 (RSI:{s.rsi:.1f}, ADX:{s.adx:.1f})")
            ...
            if i % 10 == 0:
                ...
                print(f"   [{date}] 포트폴리오: {total_value:,.0f}원 ({return_pct:+.2f}%) 보유: {len(self.positions)}종목")
    ...
    def generate_report(self) -> Dict:
        ...
        annual_volatility = np.std(daily_returns) * np.sqrt(252) * 100 if daily_returns else 0
...
def main():
    ...
    bt = ExplosiveStrategyBestScenario()
    results = bt.run_backtest()
    
    # 결과 저장
    output_path = '/root/.openclaw/workspace/strg/docs/explosive_best_scenario_h_results.json'
